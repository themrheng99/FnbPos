<?php
include '../_base.php';
$p->title = 'Update Customer Status by All the Admin or Staff';
...
if($p->get) {
    $id = $p->req('id');
    if ($id == 1) $p->redirect('../home.php');

    $stm = $db->prepare('SELECT * FROM customer WHERE id = ?');
    $stm->execute([$id]);
    $m = $stm->fetch();
...
if ($p->post) {
    ...
    if ($v->valid()) {
        // update user
        $stm = $db->prepare('UPDATE customer SET `status` = ?
                             WHERE id = ?');
